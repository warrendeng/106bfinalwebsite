...
marker_size_goal = determine_marker_size(r, 0.1);
font_size = determine_font_size(r, 0.08);
title_font = determine_font_size(r,0.09);
font_size_iteration = determine_font_size(r,0.05);
line_width = 3;
...
for i = 1:length(waypoints)
    ...
    g(i) = plot(waypoints(1,i), waypoints(2,i),'s-','MarkerSize',marker_size_goal,'LineWidth',line_width,'Color',CM);
...
%Iteration Plot setup
iteration_caption = sprintf('Iteration: %d', 0);
time_caption = sprintf('Total Time Elapsed: %0.2f', toc(start_time));
title_project = sprintf('Safety using control barrier function');

iteration_label = text(-1.8, -1.2, iteration_caption, 'FontSize', font_size_iteration, 'Color', 'r', 'FontWeight', 'bold');
time_label = text(-1.8, -1.3, time_caption, 'FontSize', font_size_iteration, 'Color', 'r', 'FontWeight', 'bold');
title = text(-1.8, 1.3, title_project, 'FontSize',title_font, 'Color', 'b', 'FontWeight', 'bold');
uistack([iteration_label], 'top'); % Iteration label is on top.
uistack([time_label],'top');
...
%     save('data.mat','t1','dxu','-append');
%     t1 = toc(start_time);
%     dxu = uni_barrier_cert(dxu, x);
%     save('data_barrier.mat','t1','dxu','-append');
    ...
    for q = 1:N-1
        follower_labels{q}.Position = x(1:2, q+1) + [-0.05;0.05];    
    end
    ...
    marker_size_goal = num2cell(ones(1,length(waypoints))*determine_marker_size(r, 0.03));
    x = marker_size_goal{:};
    %[g.MarkerSize]
    ...
    %% UPDATE ITERATION AND TIME CAPTIONS and Labels
    ...
    % size, this is unnecessary in submission as the figure window 
    % does not change size)
    iteration_label.FontSize = font_size_iteration;
    time_label.FontSize = font_size_iteration;
    r.step();
end
